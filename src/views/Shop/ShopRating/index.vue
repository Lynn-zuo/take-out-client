<template>
  <div class="ratings" ref="shopRatings">
          ...
          <li class="rating-item" v-for="(rating,index) in filterRatings" :key="index">
            ...
            <div class="content">
              ...
              <div class="star-wrapper">
                ...
                <span class="delivery">{{rating.deliveryTime || 0}}</span>
              </div>
...
export default {
  ...
  mounted() {
    this.$store.dispatch('getShopRatings', () => {
      this.$nextTick(() => {
        new BScroll('.ratings',{ // 'this.$refs.shopRatings'
          click: true
        })
      })
    })
  ...
  computed:{
    ...
    filterRatings() {
      // 得到相关的数据
      const {shopRatings, onlyContent, selectType} = this
      // 产生一个过滤新数组
      return shopRatings.filter(rating => {
        ...
        条件1:
          selctType: 0/1/2
          rateType: 0/1
          slectType===2 || selectType===rateType
        条件2:
          onlyTextShow:true/false
          text:有值/没值
          !onlyTextShow || text.length > 0
        */
